...
class LogMelFilterBanks(nn.Module):
    ...
    def _init_melscale_fbanks(self):
        ...
        # Number of frequency bins is n_fft // 2 + 1 for one-sided spectrum
    ...
    def forward(self, x):
        ...
        # mel_fbanks shape: (n_freqs, n_mels)
        # power_spec shape: (batch, n_freqs, n_frames)
        # We need to transpose mel_fbanks to (n_mels, n_freqs) for matrix multiplication
        mel_spec = torch.matmul(power_spec.transpose(-1, -2), self.mel_fbanks.to(x.device)).transpose(-1, -2)
